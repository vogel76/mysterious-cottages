...
export function AudioPlayer({ src, title }: { src: string; title: string }) {
  ...
  return (
    <div className="custom-audio">
      ...
      />
      <button className="audio-play" type="button" onClick={() => void togglePlayback()} aria-label={playing ? `Wstrzymaj: ${title}` : `Odtwórz: ${title}`}>
        {playing ? <Pause size={18} weight="fill" /> : <Play size={18} weight="fill" />}
      </button>
      <label className="audio-seek">
        <span className="sr-only">Pozycja nagrania</span>
        <input
          type="range"
          min="0"
          max={duration || 0}
      ...
      <button className="audio-mute" type="button" onClick={() => changeVolume(volume ? 0 : 1)} aria-label={volume ? 'Wycisz nagranie' : 'Włącz dźwięk'}>
